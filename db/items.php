...
class Items extends ItemsDB{
	...
	public function set_items_general($table){
		...
		// Split entire JSON into batches of 200 
		...
		*/
		...
		for ($i = 0; $i < count($batch); $i++){
			...
			foreach ($resultItems as $attr){
				...
				if ($i == count($batch) - 1 && $count == $length - 1){
					$valueSTMT .= "('$name', '$id', '$vendor', '$icon')"; 
				} else {
					$valueSTMT .= "('$name', '$id', '$vendor', '$icon'),"; 
				}
...
// Initialize map DB
$itemsDB = new Items();
//$itemsDB->set_items_general('items');
